...
output_dir = r"F:\RSFISH\script for nuclei assignment_july2026\OUTPUT_SXLGFP_FIXED"

masks_dir = r"Z:\Sujay\RNAi screen\DNA-FISH_RNAi_screen\AP_file_2026\Segmentation_masks\wt_MASK"
spots_dir = r"Z:\Sujay\RNAi screen\DNA-FISH_RNAi_screen\AP_file_2026\AP_files\sxlgfpmale dna fish_15082025\WT_SPOTDETECTED"
czi_dir   = r"Z:\Sujay\RNAi screen\DNA-FISH_RNAi_screen\AP_file_2026\AP_files\sxlgfpmale dna fish_15082025\WT"
gene_label = "SXLGFP"
...
# Fallback voxel size, used ONLY if a per-embryo CZI can't be found/read
# (this should not happen in normal operation -- if it does, that embryo is
# flagged and its results should be treated with caution).
...
voxel_log_csv = os.path.join(output_dir, "VOXEL_SIZE_USED_PER_EMBRYO.csv")


# ==========================================================
# PER-EMBRYO VOXEL SIZE (read from each embryo's own CZI metadata)
# ==========================================================
...
def get_voxel_for_embryo(embryo_base_name, voxel_log_rows):
    czi_path = os.path.join(czi_dir, embryo_base_name + ".czi")

    if not os.path.isfile(czi_path):
        print(f"    WARNING: no CZI found for voxel lookup ({czi_path}) -- using fallback voxel size")
        voxel_log_rows.append({
            "embryo": embryo_base_name, "source": "FALLBACK (CZI not found)",
...
# ==========================================================
# EXACT 3-WAY PER-NUCLEUS TRIPLET MATCHING (unchanged from validated pipeline)
# ==========================================================
...
def process_embryo(mask_path, spots_paths, embryo_name, gene_label, voxel_log_rows):
    ...
    print(f"    Voxel size used: X={vx:.5f}  Y={vy:.5f}  Z={vz:.5f}")
...
mask_files = [f for f in os.listdir(masks_dir) if f.lower().endswith("_mask.tif")]
print(f"=== Gene: {gene_label} ({masks_dir}) -- {len(mask_files)} mask file(s) ===\n")

for mask_fname in mask_files:
    if not mask_fname.endswith("_mask.tif"):
        skipped.append({"gene": gene_label, "file": mask_fname, "reason": "unexpected filename pattern"})
        print(f"  SKIP (unexpected filename casing): {mask_fname}")
        continue
    ...
    spots_paths = {
        role: os.path.join(spots_dir, base_name + suffix)
        for role, suffix in CHANNEL_SUFFIXES.items()
    }

    missing = [p for p in spots_paths.values() if not os.path.isfile(p)]
    if missing:
        skipped.append({"gene": gene_label, "file": base_name, "reason": f"missing CSV(s): {missing}"})
    ...
    try:
        ...
        print(f"    -> {stats['n_final_triplets']} final triplets "
              f"({stats['n_nuclei_dropped_cap']} nuclei dropped by cap, "
              f"{stats['n_fallback_nuclei']} fallback nuclei)")
    except Exception as e:
...
if all_final_dfs:
    df_gene = pd.concat(all_final_dfs, ignore_index=True)
else:
    df_gene = pd.DataFrame()

print(f"\nTotal final triplets: {len(df_gene)}")

if len(df_gene) > 0:
    out_path = os.path.join(output_dir, f"{gene_label}_FINAL_TRIPLETS_FIXED_VOXEL.csv")
    df_gene.to_csv(out_path, index=False)
    print(f"Saved: {out_path}")

if skipped:
    pd.DataFrame(skipped).to_csv(skip_log_csv, index=False)
    print(f"Saved skip log ({len(skipped)} embryos skipped): {skip_log_csv}")

pd.DataFrame(voxel_log_rows).to_csv(voxel_log_csv, index=False)
print(f"Saved voxel size log (per embryo): {voxel_log_csv}")

if all_stats:
    df_stats = pd.DataFrame(all_stats)
    print(f"\n=== SUMMARY ===")
    print(f"Embryos processed successfully: {len(df_stats)}")
    print(f"Embryos skipped: {len(skipped)}")
    print(f"Total final triplets: {df_stats['n_final_triplets'].sum()}")
    print(f"\nVoxel sizes used per embryo:")
    print(df_stats[["embryo", "voxel_x", "voxel_y", "voxel_z", "n_final_triplets"]].to_string(index=False))
